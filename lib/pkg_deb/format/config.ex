defmodule PkgDeb.Format.Config do
  @moduledoc """
  ...
  def build_config(%Mix.Release{} = release, pkg_config) do
    base_config =
      [
        {:name, Atom.to_string(release.name)},
        {:version, format_package_version(release.version, pkg_config)},
        {:arch, format_package_arch(pkg_config)}
      ] ++ config_from_package(pkg_config)
  ...
  defp handle_config(:description, value) do
    {:description, value}
  end
  ...
  defp check_valid(config = %PkgDeb.Format.Config{}) do
